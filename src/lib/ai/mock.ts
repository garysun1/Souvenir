...
export const mockProvider = {
  async identifyPlace(_input: IdentifyRequest): Promise<IdentifyResponse> {
  ...
  async planOuting(input: PlanRequest): Promise<PlanResponse> {
    ...
    return { title: "A Souvenir outing", summary: "A simple route assembled by the mock planner.", places: results.map((result) => result.place) };
  },
};
