...
export const openAiProvider = {
      ...
      messages: [
        {
          ...
          content: [
            {
              type: "text",
              text: "Identify this place in Los Angeles. Return a concise place name.",
            },
  ...
  async planOuting(input: PlanRequest): Promise<PlanResponse> {
    ...
    if (!client)
      return {
        title: "Suggested outing",
        summary: "OpenAI is not configured; showing search suggestions.",
        places: results.map((item) => item.place),
      };
    await client.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: `Plan an outing using these places: ${results.map((item) => item.place.name).join(", ")}`,
        },
      ],
      tools: [
        {
          ...
          function: {
            name: "search_places",
            description: "Search Souvenir places",
            parameters: {
              type: "object",
    ...
    return {
      title: "Suggested outing",
      summary: "A route assembled with OpenAI and the Souvenir search service.",
      places: results.map((item) => item.place),
    };
  },
};
