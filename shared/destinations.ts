...
export function destinationCities(
  places: { city?: string | null; country?: string | null }[],
  includeUnknownCountry = false,
) {
  const cities = new Map<string, { city: string; country: string; label: string }>();
  for (const place of places) {
    if (!place.city || (!place.country && !includeUnknownCountry)) continue;
    const country = place.country?.toUpperCase() ?? "";
    cities.set(JSON.stringify([place.city, country]), {
      city: place.city,
      country,
      label: `${place.city}, ${countryName(country) || "country not recorded"}`,
    });
  }
