...
if (!process.argv.includes("--allow-disposable-db")) {
  throw new Error(
    "This runner applies migrations only to a new local Docker database. Explicit authorization requires --allow-disposable-db.",
  );
}
...
try {
  ...
  if (new URL(url).hostname !== "127.0.0.1") throw new Error("Non-loopback database refused.");
  ...
  execFileSync(
    "corepack",
    ["pnpm", "exec", "vitest", "run", "--config", "tests/memories/vitest.integration.config.ts"],
    {
      cwd: root,
      stdio: "inherit",
      env: {
