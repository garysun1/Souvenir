...
export function PlacePhoto({ placeId, uri, style, children }: { placeId: string; uri?: string; style?: StyleProp<ViewStyle>; children?: React.ReactNode }) {
  ...
  const source = failed === identity ? undefined : sample ? placeImages[sample] : uri ? localUri ? { uri: localUri } : undefined : placeImages[place?.fixtureId ?? placeId] ?? (place?.heroImageUrl ? { uri: place.heroImageUrl } : undefined);
  ...
  return <View style={[{ backgroundColor: colors.brandSoft, overflow: 'hidden', borderRadius: 12 }, style]}>
    {source ? <Image source={source} cachePolicy={edition ? 'none' : 'disk'} contentFit="cover" transition={180} onError={() => { setFailed(identity); if (edition && !retry) setRetry(1); }} style={{ width: '100%', height: '100%', position: 'absolute' }} accessibilityLabel={uri && !sample ? 'Personal visit photograph' : 'Destination photograph'} /> : <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 6, padding: 12 }}><Icon name="image" size={32} /><T variant="small" muted>{loading ? 'Loading your photo…' : uri && !sample ? 'Personal photo unavailable' : 'Photo unavailable'}</T>{uri && !sample && !loading && <T variant="small" muted style={{ textAlign: 'center' }}>Your visit details are still here.</T>}{edition && <Button label="Retry photo" variant="ghost" onPress={() => setRetry(value => value + 1)} />}</View>}
    {children}
  </View>;
}
