...
import { collectedPlaceIds, latestEdition, visitDate } from '@/state/selectors';
import { sets, placeById } from '@/fixtures/catalog';
...
export default function SetDetail() {
  ...
  return <Screen><Header back title={downtownSet.title} subtitle={`${downtownSet.placeIds.length} places`} /><PlacePhoto placeId={downtownSet.placeIds[0] ?? ''} style={{ height: 220 }} />
    ...
    {count > 0 && count === downtownSet.placeIds.length && <Button label="View your collection" variant="outline" onPress={() => router.push('/collection')} style={{ marginTop: 22 }} />}
    <Sheet visible={completion} onClose={() => setCompletion(false)} title={`${downtownSet.title} complete`}><View style={styles.badge}><Icon name="building" size={42} /><T variant="title" color={colors.brand}>{downtownSet.title}</T><T muted style={{ textAlign: 'center' }}>Collected as one chapter.</T></View><Button label="Keep exploring" onPress={() => setCompletion(false)} /></Sheet>
  </Screen>;
}
