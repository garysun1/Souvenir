...
import { editionMonths, recommendationLabel, wishlistMemberships } from '@/domain/collection';
import { collectedPlaceIds, latestEdition, ownEditions, savedPlaceIds, visitDate } from '@/state/selectors';
...
export function BeenList({ state, places }: { state: AppState; places: Place[] }) {
  return <View>{places.map(place => {
    const editions = ownEditions(state).filter(edition => edition.placeId === place.id);
...
export function CollectionEmpty({ section, narrowed }: { section: 'been' | 'saved'; narrowed: boolean }) {
  return <EmptyState icon={section === 'been' ? 'image' : 'bookmark'} title={narrowed ? 'No matching places' : section === 'been' ? 'Your album is ready' : 'No saved places yet'} message={narrowed ? 'Clear or change your collection filters.' : section === 'been' ? 'Capture a visit or bring in sample memories from Dropbox.' : 'Save a place from Discover and it will stay here.'} action={section === 'been' && !narrowed ? 'Import memories' : 'Explore places'} onPress={() => router.push(section === 'been' && !narrowed ? '/import/dropbox' : '/discover')} />;
}
