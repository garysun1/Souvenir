...
export function CollectionFilterBar({ filters, onChange }: { filters: CollectionFilters; onChange: (filters: CollectionFilters) => void }) {
  ...
  const categoryLabel = filters.category === 'all' ? 'Category' : filters.category === 'cultural' ? 'Culture' : filters.category === 'park' ? 'Parks' : 'Landmarks';
  return <>
    <View style={{ flexDirection: 'row', alignItems: 'center' }}><View style={{ flex: 1 }}><ChipRow><Chip label={categoryLabel} selected={filters.category !== 'all'} icon="down" onPress={() => { setDraft(filters); setOpen(true); }} /><Chip label="Favorites" icon="heart" selected={filters.favorites} onPress={() => onChange({ ...filters, favorites: !filters.favorites })} /><Chip label={filters.companion ? `With ${users.find(user => user.id === filters.companion)?.name}` : 'Companion'} icon="people" selected={!!filters.companion} onPress={() => { setDraft(filters); setOpen(true); }} /><Chip label="Visit date" icon="clock" selected={!!filters.after || !!filters.before} onPress={() => { setDraft(filters); setOpen(true); }} /><Chip label={`Sort: ${filters.sort === 'recent' ? 'Recent' : filters.sort === 'name' ? 'Name' : 'Your rank'}`} icon="down" selected={filters.sort !== 'recent'} onPress={() => { setDraft(filters); setOpen(true); }} /></ChipRow></View>{filtersActive(filters) && <Button label="Clear" variant="ghost" onPress={() => onChange(defaultCollectionFilters)} />}</View>
    <Sheet visible={open} onClose={() => setOpen(false)} title="Filter collection">
      <T variant="label">Category</T><ChipRow>{(['all', 'cultural', 'park', 'landmark'] as const).map(category => <Chip key={category} label={category === 'all' ? 'All' : category === 'cultural' ? 'Culture' : category === 'park' ? 'Parks' : 'Landmarks'} selected={draft.category === category} onPress={() => setDraft({ ...draft, category: category as Category | 'all' })} />)}</ChipRow>
      <T variant="label">Companion</T><ChipRow><Chip label="Anyone" selected={!draft.companion} onPress={() => setDraft({ ...draft, companion: '' })} />{users.filter(user => user.id !== 'you').map(user => <Chip key={user.id} label={user.name} selected={draft.companion === user.id} onPress={() => setDraft({ ...draft, companion: user.id })} />)}</ChipRow>
